...
const animationTimeline = () => {
    ...
    // 🎂 Call Cake Cutting Ceremony BEFORE the photo appears
    .add(() => { showCakeCeremony(); }, "+=1.5")

    .staggerFromTo(".baloons img", 2.5, { opacity: 0.9, y: 1400 }, { opacity: 1, y: -1000 }, 0.2)

    // 📸 Photo now appears AFTER the cake-cutting ceremony
...
function showCakeCeremony() {
    ...
    // Make the cake visible
    setTimeout(() => {
        cake.style.opacity = "1"; // Fade in cake
    }, 500);

    setTimeout(() => {
        // Move knife to cutting position
        knife.style.transform = "translateX(-40px) translateY(50px) rotate(0deg)";
    }, 1000);

    setTimeout(() => {
        // Slight shake before slicing
        knife.style.transform = "translateX(-42px) translateY(52px) rotate(-10deg)";
    }, 1400);

    setTimeout(() => {
        // Knife cuts cake
        knife.style.transform = "translateX(-40px) translateY(80px) rotate(45deg)";
        cakeSlice.style.opacity = "1"; // Show cake slice
        cakeSlice.style.transform = "translateX(40px) translateY(-20px) rotate(10deg)"; // Move slice out
    }, 1800);

    setTimeout(() => {
        // Hide knife after cutting
        knife.style.opacity = "0";
    }, 2500);

    setTimeout(() => {
        // Fade out cake and cake slice after cutting
        cake.style.opacity = "0";
        cakeSlice.style.opacity = "0";
    }, 3500);

    // Play cutting sound
    const cutSound = new Audio("cut-sound.wav");
    cutSound.play();
}